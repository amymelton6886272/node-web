export const policyPages = {
  ...
  contact: {
    ...
    sections: [
      ...
      { title: 'What we can and cannot fix', titleZh: '我们能修什么、不能修什么', text: 'We can fix broken pages, outdated guide wording, and tool UI bugs on Storewise. We cannot cancel your Apple subscription, approve a refund, unlock an Apple ID, or intervene in bank disputes. For those, use Apple Support, Report a Problem, or your payment provider.', textZh: '我们可以修复 Storewise 坏页、过时指南措辞与工具 UI Bug。不能取消你的 Apple 订阅、批准退款、解锁 Apple ID 或介入银行争议。那些请走 Apple 支持、Report a Problem 或支付机构。' },
],
    checklist: {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: '2. Local browser data', titleZh: '2. 浏览器本地数据', text: 'Language preference, theme choice, checklist progress, temporary UI state, and temporary interface state may be stored in your browser to improve usability. You can clear this information through your browser settings at any time.', textZh: '语言偏好、主题选择、清单进度和临时界面状态可能会保存在你的浏览器中，以提升使用体验。你可以随时通过浏览器设置清除这些内容。' },
      { title: '3. Third-party requests', titleZh: '3. 第三方请求', text: 'Some tools may contact Apple public APIs, Apple-hosted media resources, public promotional feeds, or links that you intentionally open. Those external services may receive normal request metadata such as IP address, user agent, referrer, or request timing under their own policies.', textZh: '部分工具可能会请求 Apple 公开 API、公开元数据服务、Apple 托管的媒体资源、公开促销源，或你主动打开的外部链接。这些服务可能会根据其自身政策接收常规请求元数据，例如 IP 地址、浏览器标识、来源页和请求时间。' },
      ...
      { title: '5. Interest-based ads and third-party vendors', titleZh: '5. 兴趣广告与第三方广告商', text: 'Third-party vendors, including Google, use cookies to serve ads based on a user\'s prior visits to this website or other websites. Google\'s use of advertising cookies enables it and its partners to serve ads to users based on their visit to this site and/or other sites on the Internet. Users may opt out of personalized advertising by visiting Google Ads Settings at https://adssettings.google.com/. Alternatively, users can opt out of some third-party vendors\' use of cookies for personalized advertising by visiting https://www.aboutads.info/choices/. For more information about how Google uses data when you use our partners\' sites or apps, see https://policies.google.com/technologies/partner-sites.', textZh: '包括 Google 在内的第三方供应商会使用 Cookie，根据用户此前对本网站或其他网站的访问情况投放广告。Google 使用广告 Cookie，可使其自身及其合作伙伴根据用户对本站和/或互联网上其他网站的访问情况投放广告。用户可访问 Google 广告设置（https://adssettings.google.com/）退出个性化广告；也可访问 https://www.aboutads.info/choices/ 退出部分第三方供应商将 Cookie 用于个性化广告。关于您在使用合作伙伴网站或应用时 Google 如何使用数据，请参阅 https://policies.google.com/technologies/partner-sites。' },
      ...
      { title: 'Advertising placement intent', titleZh: '广告位放置意图', text: 'When ads are enabled, Storewise intends to place them near substantial explanations rather than on empty result canvases. Third-party ad networks still control delivery and measurement under their policies. If you block ads or third-party cookies, research articles should remain readable.', textZh: '启用广告时，Storewise 倾向于把广告放在有实质说明的区域附近，而不是空结果画布。第三方广告网络仍按其政策控制投放与衡量。若你拦截广告或第三方 Cookie，研究文章仍应保持可阅读。' },
],
    checklist: {
      ...
      items: [
        ...
        { text: 'Google AdSense cookie and interest-based ad disclosures are listed above, with opt-out links', textZh: '上文已列出 Google AdSense Cookie 与兴趣广告说明，并提供退出链接' },
      ],
    },
    links: [
  ...
  disclaimer: {
    ...
    sections: [
      ...
      { title: '3. Verify before acting', titleZh: '3. 操作前请自行核验', text: 'Prices, subscriptions, app metadata, region availability, and third-party links can change. Verify critical information through official sources before acting.', textZh: '价格、订阅规则、应用元数据、地区可用性、IP 地理位置和第三方链接都可能变化。购买、订阅、切区、安装应用或授权权限前，请通过官方来源再次确认。' },
      { title: '4. Tool limitations', titleZh: '4. 工具结果的限制', text: 'Search results, app risk scores, checklists, and research notes are aids for review, not guarantees. Missing data, cached responses, API changes, regional differences, or browser restrictions may affect results.', textZh: '搜索结果、生成的测试数据、应用风险评分、检查清单和网络诊断都只是辅助判断，并不是保证。数据缺失、缓存、API 变化、地区差异或浏览器限制都可能影响结果。' },
  ...
  terms: {
    ...
    sections: [
      ...
      { title: 'Acceptable research use', titleZh: '可接受的研究用途', text: 'You may use Storewise to research App Store purchases and learn household safety workflows. You may not use the site or any example data to commit fraud, evade platform rules, or misrepresent identity. Automated scraping that degrades public APIs or the site may be rate-limited or blocked.', textZh: '你可用 Storewise 研究 App Store 购买并学习家庭安全流程。不得利用本站或任何示例数据实施欺诈、规避平台规则或虚假身份。损害公开 API 或本站的自动化抓取可能被限速或拦截。' },

      { title: 'No reverse engineering of Apple systems', titleZh: '不鼓励逆向 Apple 系统', text: 'Do not use Storewise to attempt unauthorized access, credential stuffing, payment fraud, or abuse of public APIs. Rate limits and access controls may apply. Continued abuse can result in blocked access.', textZh: '不得利用 Storewise 尝试未授权访问、撞库、支付欺诈或滥用公开 API。可能适用速率限制与访问控制。持续滥用可能导致访问被阻止。' },

      { title: 'Changes to the service', titleZh: '服务变更', text: 'Tools, guides, and navigation may change as the project focuses on higher-value App Store research workflows. Continued use after updates constitutes acceptance of the revised terms and page set.', textZh: '随着项目更聚焦高价值的 App Store 研究流程，工具、指南与导航可能变更。更新后继续使用即表示接受修订后的条款与页面集合。' },
],
    checklist: {
  ...
  'data-sources': {
    title: 'Data Sources',
    titleZh: '数据来源',
    sub: 'Where Storewise may obtain App Store metadata, promotional feeds, and safety reference information.',
    subZh: '说明 Storewise 可能从哪里获取应用、价格、促销与安全参考信息，以及这些数据的限制。',
    effective: 'Effective date: 2026-07-15',
    effectiveZh: '生效日期：2026-07-15',
    sections: [
      ...
      { title: '3. Network metadata (historical note)', titleZh: '3. 网络元数据（历史说明）', text: 'Older builds experimented with network-exit lookup. That utility is no longer a product entry point. Any remaining educational mention of network metadata is historical and should not be read as an active Storewise tool.', textZh: '旧版本曾试验过网络出口查询。该工具已不再作为产品入口。若文中仍出现网络元数据相关教育性说明，仅作历史背景，不应理解为 Storewise 仍提供该工具。' },
      { title: '4. Example and synthetic data', titleZh: '4. 示例与合成数据', text: 'Any synthetic or example data shown for illustration is not a real identity, mailing address, or payment credential. Do not use example data for verification, billing, fraud, or platform bypass attempts.', textZh: '本站用于说明的合成或示例数据不是真实身份、邮寄地址或支付凭证。不得用于身份验证、账单、欺诈或绕过平台验证。' },
      ...
      { title: 'Freshness and failure handling', titleZh: '新鲜度与失败处理', text: 'App metadata and free lists can fail or lag. When a feed is empty, pages keep explanatory content, limitations, and related guides instead of collapsing into a blank canvas. Users should re-check official storefronts for final prices and availability. Storewise optimizes for decision support under partial data, not for pretending every API always succeeds.', textZh: '应用元数据、限免列表与网络检测可能失败或滞后。源为空时，页面仍保留说明、限制与相关指南，而不是塌成空白画布。最终价格与可用性请重核官方店面。Storewise 优化的是部分数据下的决策支持，而不是假装每个 API 永远成功。' },
