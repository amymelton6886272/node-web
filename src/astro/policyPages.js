export const policyPages = {
  about: {
    ...
    subZh: '一个独立的 Apple / iOS 资源站，结合实用工具、安全内容与透明的数据来源说明。',
    highlights: [
      { label: 'Focus', labelZh: '定位', value: 'Apple / iOS utility hub', valueZh: 'Apple / iOS 实用工具站' },
      { label: 'Approach', labelZh: '方式', value: 'Static-first, low-friction', valueZh: '静态优先，轻量直接' },
      { label: 'Content', labelZh: '内容', value: 'Tools, guides, articles, policies', valueZh: '工具、指南、文章、政策页' },
    ],
    sections: [
      { title: 'What Wolffy is', titleZh: 'Wolffy 是什么', text: 'Wolffy is a lightweight web toolkit for Apple and iOS users. It brings together App Store price comparison, in-app purchase lookup, free-app discovery, icon lookup, IP checks, test address generation, app risk assessment, decision checklists, and long-form safety articles.', textZh: 'Wolffy 是面向 Apple 与 iOS 用户的轻量级网站工具箱，整合了 App Store 价格对比、内购查询、限免发现、图标识别、IP 检测、测试地址生成、应用风险评估、决策清单和长文指南。' },
      { title: 'Why it exists', titleZh: '为什么要做这个站', text: 'Many Apple-related decisions require jumping across App Store pages, Apple account settings, regional storefronts, privacy labels, refund rules, and third-party references. Wolffy tries to organize those scattered signals into pages that are easier to scan and verify before you pay, subscribe, switch regions, or grant permissions.', textZh: '很多 Apple 相关决策都需要反复切换 App Store 页面、Apple 账户设置、地区商店、隐私标签、退款说明和第三方资料。Wolffy 试着把这些分散信息整理成更容易浏览和核对的页面，帮助你在付款、订阅、切区或授权权限前先看明白。' },
      { title: 'How it is maintained', titleZh: '网站如何维护', text: 'Wolffy is maintained as an independent project with a preference for static pages, client-side tools, and public data references. That keeps pages fast, easier to audit, and less invasive for visitors. The goal is usefulness and clarity rather than account-heavy workflows.', textZh: 'Wolffy 作为独立项目维护，优先使用静态页面、浏览器端工具和公开数据来源。这样页面更快、更容易检查，也更少打扰访问者。整体目标是让网站实用、清楚，而不是堆砌复杂账号流程。' },
      ...
      { title: 'What Wolffy is not', titleZh: 'Wolffy 不是什么', text: 'Wolffy is not an official Apple property, not a payment platform, not an account service, and not a legal or financial advisor. It is a research helper meant to support safer and more informed user choices.', textZh: 'Wolffy 不是 Apple 官方产品，不是支付平台，不是账号服务，也不是法律或财务顾问。它更像一个研究辅助站，用来帮助用户做出更稳妥的信息判断。' },
    ],
    checklist: {
  ...
  contact: {
    ...
    sections: [
      ...
      { title: 'Privacy and partnership requests', titleZh: '隐私与合作请求', text: 'For privacy-related questions or advertising and partnership inquiries, describe the page involved and the type of request you are making. Wolffy does not require registration, so most preference data lives in the browser rather than in a user account on a server.', textZh: '如果是隐私相关问题，或者广告与合作咨询，请说明涉及页面和请求类型。Wolffy 不要求注册账号，所以大多数偏好数据保存在浏览器中，而不是服务器用户账户里。' },
  ...
  privacy: {
    ...
    sections: [
      { title: '1. Overview', titleZh: '1. 概览', text: 'Wolffy is primarily a client-side website. Most pages load as static HTML, JavaScript, and CSS. The site does not ask users to register, sign in, create profiles, or submit passwords just to browse public content and use basic tools.', textZh: 'Wolffy 主要是一个浏览器端网站。大多数页面以静态 HTML、JavaScript 和 CSS 方式加载。用户访问公开内容和基础工具时，不需要注册、登录、创建资料或提交密码。' },
      ...
      { title: '4. Advertising and analytics considerations', titleZh: '4. 广告与统计说明', text: 'If Google AdSense or similar advertising services are enabled in the future, those providers may use cookies, local storage, IP address, page URL, and browser information according to their own policies. Wolffy aims to keep ads out of empty-result screens and away from deceptive placements, but third-party ad behavior is still governed by the provider.', textZh: '如果未来启用 Google AdSense 或类似广告服务，这些提供方可能会根据自身政策使用 Cookie、本地存储、IP 地址、页面 URL 和浏览器信息。Wolffy 会尽量避免把广告放在空结果页或容易误导点击的位置，但第三方广告的具体行为仍由服务提供方决定。' },
