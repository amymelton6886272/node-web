export const toolContent = {
  en: {
    price: {
      ...
      items: [
        ...
        { heading: 'How the comparison works', text: 'Wolffy queries public App Store metadata for the selected regions, normalizes currency display, and keeps the original storefront price visible next to any converted estimate. Availability state is shown separately so a missing region is not mistaken for a zero price. When an interface fails, the page should keep explanatory content instead of collapsing into an empty search box.' },
  ...
  zh: {
    ...
    appfree: {
      title: '限免应用页面的筛选原则', intro: '限免信息变化很快，单纯复制第三方列表容易被视为低价值聚合。本站会补充应用分类、开发者、可用链接和使用提醒，帮助用户判断是否值得下载，而不是只展示一组外链。', items: [{ heading: '限免不等于永久免费', text: '很多应用只在短时间内免费，恢复原价后可能仍显示历史缓存。打开 App Store 前请再次确认价格，尤其是订阅型应用和包含内购的应用。' }, { heading: '优先查看应用质量信号', text: '建议结合开发者名称、评分、更新时间、隐私标签和评论趋势判断。没有维护、权限异常或描述夸张的应用，即使限免也不一定值得安装。' }, { heading: '保留上下文内容', text: '当接口暂时失败或当天数据为空时，页面仍应显示说明、筛选建议和相关文章，避免形成“无发布商内容但展示广告”的屏幕。' }, { heading: '限免信息应如何整理', text: '有价值的限免页会补充分类、开发者身份、更新活跃度、隐私负担，以及应用是真免费还是仅免费安装。目标是辅助决策，而不是堆外链。数据不完整时，页面应明确说明。' }, { heading: '安装限免应用前', text: '再次确认 App Store 当前价格，查看近期评论是否有扣费投诉，检查核心功能是否依赖订阅，并核对权限。如果一个简单工具索要高权限，即使免费下载也应跳过。' }], links: [{ href: '/price', label: '价格对比' }, { href: '/iap', label: '检查内购' }, { href: '/guides', label: '限免应用指南' }, { href: '/risk', label: 'App 风险评估' }], linksLabel: '相关阅读'
    },
    iap: {
