...
const relatedPages={price:['appfree','ip','guides','glossary'],appfree:['price','iap','guides','glossary'],ip:['address','proxy','guides'],address:['ip','guides'],iap:['price','appfree','icon','guides'],icon:['iap','guides'],proxy:['ip','guides']};export function ToolIntro({page}){const {t}=useLang(); const data=t[page]; const intro=data?.intro; const tips=data?.tips; const how=data?.howItWorks; const ts=data?.troubleshooting; const rel=relatedPages[page]; if(!intro&&!tips&&!how&&!ts&&!rel)return null; return <section className="toolIntro card">{intro&&<><h3>{intro.heading}</h3>{[intro.p1,intro.p2,intro.p3,intro.p4].map((p,i)=><p key={i}>{p}</p>)}</>}{intro&&(tips||how||ts)&&<AdBanner/>}{how&&<div className="toolHow"><h3>{how.heading}</h3>{[how.p1,how.p2,how.p3].map((p,i)=><p key={i}>{p}</p>)}</div>}{tips&&<div className="toolTips"><h3>{tips.heading}</h3><ul>{tips.items.map((tip,i)=><li key={i}>{tip}</li>)}</ul></div>}{ts&&<div className="toolTrouble"><h3>{ts.heading}</h3><ul>{ts.items.map((item,i)=><li key={i}>{item}</li>)}</ul></div>}{rel&&<div className="toolRelated"><span>{t.search?.categories?.guides||'See also'}:</span>{rel.map(id=>{const href='/'+id;return <a key={id} href={href} onClick={e=>{e.preventDefault();history.pushState({},'',href);window.dispatchEvent(new PopStateEvent('popstate'))}}>{t.nav?.[id]||t.glossary?.title||id}</a>})}</div>}</section>}
...
export function SearchModal({open,onClose}){const {t}=useLang();const[q,setQ]=useState('');const inp=useRef(null);const idx=useMemo(()=>buildSearchIndex(t),[t]);const results=useMemo(()=>{if(!q.trim())return[];const lq=q.toLowerCase();return idx.filter(r=>r.text.toLowerCase().includes(lq)||r.title.toLowerCase().includes(lq)).slice(0,10)},[q,idx]);
  ...
  if(!open)return null;
  const go=url=>{const path=url.replace('#','/');history.pushState({},'',path);window.dispatchEvent(new PopStateEvent('popstate'));onClose()};
