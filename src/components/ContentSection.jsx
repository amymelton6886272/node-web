      ...
      {links.length > 0 && (
        <div className="contentLinks">
          ...
          {links.map((link) => {
            ...
            return <a key={link.href} href={href}>{link.label}</a>;
          })}
        </div>
      )}
    </section>
...
export const contentByPage = {
  home: {
    title: 'Storewise 的内容定位与使用边界',
    intro: '本站围绕 Apple / iOS 用户的查询、学习和决策流程整理工具与知识库。每个功能页面都包含说明、适用场景、限制和相关指南，帮助用户在操作前理解数据来源和使用风险。',
    items: [
      { heading: '面向真实问题，而不是堆叠链接', text: '价格对比、限免发现、内购识别、图标检索、IP 检测和地址生成都对应具体使用场景。页面会说明结果如何产生、哪些信息来自 Apple 或公开接口，以及哪些内容只适合作为参考。' },
      { heading: '工具结果与原创说明并重', text: '每个工具页默认提供教程、常见问题和操作建议。即使用户尚未搜索，也能先读到完整说明，再决定是否继续查询。' },
      { heading: '广告与内容分离', text: '如展示广告，会尽量放在正文、教程或结果列表之后，并使用明确的“广告”标识，避免干扰阅读与操作。' },
    ],
    links: [
  ...
  appfree: {
    title: '限免应用页面的筛选原则',
    intro: '限免信息变化很快。本站会补充应用分类、开发者、可用链接和使用提醒，帮助用户判断是否值得下载，而不是只展示一组外链。',
    items: [
      ...
      { heading: '保留上下文内容', text: '当接口暂时失败或当天数据为空时，页面仍会显示说明、筛选建议和相关文章，方便你改关键词或稍后再试。' },
    ],
    links: [
  ...
  icon: {
    ...
    items: [
      ...
      { heading: '搜索为空时的处理', text: '如果没有结果，页面仍提供使用说明和相关指南，方便你调整关键词或换一种检索方式。' },
    ],
    links: [
