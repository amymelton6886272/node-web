...
function Layout(){
 ...
 return <><header className="top"><a className="brand" href="/" onClick={e=>{e.preventDefault();navigateTo("home")}}><span className="logo" aria-label="Wolffy logo">W</span><b>Wolffy</b></a><nav className="toplinks"><a href="/" onClick={e=>{e.preventDefault();navigateTo("home")}}>{t.header.home}</a><a href="/about" onClick={e=>{e.preventDefault();navigateTo("about")}}>{t.header.about}</a><a href="/contact" onClick={e=>{e.preventDefault();navigateTo("contact")}}>{t.header.contact}</a><a href="/disclaimer" onClick={e=>{e.preventDefault();navigateTo("disclaimer")}}>{t.header.disclaimer}</a><a href="/privacy" onClick={e=>{e.preventDefault();navigateTo("privacy")}}>{t.header.privacy}</a><a href="https://hub.souk.eu.org/" target="_blank" rel="noreferrer">{t.header.hub}</a></nav><div className="headerActions"><button className="searchBtn" onClick={()=>setSearchOpen(true)} title="Search (Ctrl+K)"><Search size={16}/></button><button className="lang" onClick={toggle} title="Switch language"><Languages size={16}/><span>{lang==='zh'?'EN':'中'}</span></button><button className="theme" onClick={()=>setDark(!dark)}>{dark?<Sun size={18}/>:<Moon size={18}/>}</button></div></header><div className="shell"><aside>{features.map(({id,Icon})=><a key={id} className={page===id?'active':''} href={`/${id}`} onClick={e=>{e.preventDefault();navigateTo(id)}}><Icon size={17}/>{t.nav[id]}</a>)}</aside><main><Current /></main></div><footer><span>{t.footer.builtWith}</span><span>Wolffy © 2026 · {t.footer.contact}: <a href="https://t.me/Wolffy_chat_bot" target="_blank" rel="noreferrer">@Wolffy_chat_bot</a> · <a href="/disclaimer" onClick={e=>{e.preventDefault();navigateTo("disclaimer")}}>{t.footer.disclaimer}</a> · <a href="/privacy" onClick={e=>{e.preventDefault();navigateTo("privacy")}}>{t.footer.privacy}</a></span></footer><SearchModal open={searchOpen} onClose={()=>setSearchOpen(false)}/></>
}
